              ...
              {loading ? (
                ...
                users.map((u) => (
                  <tr key={u.id} className="border-b border-zinc-50 hover:bg-zinc-50 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <span className={`inline-block text-xs font-medium px-2.5 py-1 rounded-full border ${planStyle(u.plan)}`}>
                        {planLabel(u.plan)}
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      {u.tenant ? (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-sm text-zinc-700">{u.tenant.name || u.tenant.slug}</span>
                        </div>
                      ) : (
                    ...
                    <td className="px-5 py-4 text-sm text-zinc-600">
                      {u.planExpiresAt ? format(new Date(u.planExpiresAt), "dd/MM/yyyy") : "—"}
                    </td>
