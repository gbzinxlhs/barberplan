        ...
        <div className="bg-white rounded-xl border border-zinc-200 shadow-sm mb-8 overflow-hidden max-w-lg">
          <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100">
            <h2 className="text-sm font-semibold text-zinc-900">
              {editId ? "Editar Barbeiro" : "Novo Barbeiro"}
            </h2>
          </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {barbers.map((barber, idx) => {
            ...
            return (
              <div
                key={barber.id}
                className={`group bg-white rounded-xl border border-zinc-200 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 ${
                  !barber.active ? "opacity-50" : ""
                }`}
              >
                <div className="p-6">
                  <div className="flex items-center gap-4 mb-4">
                    <div className={`w-12 h-12 rounded-full ${color} flex items-center justify-center text-sm font-bold text-white`}>
                      {initials}
                    </div>
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-zinc-100">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => toggleActive(barber.id, barber.active)} className={`text-xs h-8 ${
                      barber.active ? "text-amber-600 hover:text-amber-700" : "text-emerald-600 hover:text-emerald-700"
                    }`}>
